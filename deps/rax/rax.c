...
static inline raxNode *raxGetChild(rax *rax, raxNode **child_ptr)
{
    if (rax->urcu) {
        return __atomic_load_n(child_ptr, __ATOMIC_ACQUIRE);
    } else {
        raxNode *c;
        memcpy(&c, child_ptr, sizeof(c));
        return c;
    }
}

static inline void raxSetChild(rax *rax, raxNode **child_ptr, raxNode *child)
{
    if (rax->urcu) {
        __atomic_store_n(child_ptr, child, __ATOMIC_RELEASE);
    } else {
        memcpy(child_ptr, &child, sizeof(child));
    }
}
...
#define raxNodeCurrentLength(n)                                                                                                    \
    (sizeof(raxNode) + (n)->size + raxPadding((n)->size) + ((n)->iscompr ? sizeof(raxNode *) : sizeof(raxNode *) * (n)->size) +    \
...
void raxFreeNode(rax *rax, raxNode *n)
{
    ...
    if (rax->urcu) {
        urcu_retire((urcu_t *)rax->urcu, n, NULL);
    } else {
        rax_free(n);
    }
}
...
raxNode *raxNodeRealloc(rax *rax, raxNode *n, size_t newsize)
{
    ...
    if (rax->urcu) {
        raxNode *newn = rax_malloc_usable(newsize, &usable);
        if (newn == NULL)
            return NULL;
        size_t old_size = raxNodeCurrentLength(n);
        size_t copy_size = (old_size < newsize) ? old_size : newsize;
        memcpy(newn, n, copy_size);
        if (rax->alloc_size) {
            old_usable = malloc_usable_size(n);
...
{
    if (data == NULL)
        return n; /* No reallocation needed, setting isnull=1 */
...
raxNode *raxAddChild(rax *rax, raxNode *n, unsigned char c, raxNode **childptr, raxNode ***parentlink)
{
    ...
    raxSetChild(rax, childfield, child);
    *childptr = child;
 ...
 * its parents nodes). */
static inline size_t raxLowWalk(rax *rax, unsigned char *s, size_t len, raxNode **stopnode, raxNode ***plink, int *splitpos,
                                raxStack *ts)
{
    raxNode *h = rax->urcu ? __atomic_load_n(&rax->head, __ATOMIC_ACQUIRE) : rax->head;
    ...
    while (h->size && i < len) {
        ...
        if (h->iscompr)
            j = 0; /* Compressed node only child is at index 0. */
        h = raxGetChild(rax, children + j);
...
static int raxLinkStillValid(rax *rax, unsigned char *s, size_t len, raxNodeLink *link)
{
    ...
    return stopnode == link->stopnode && parentlink == link->parentlink && consumed == link->consumed && splitpos == link->splitpos;
}
#endif
...
int raxInsertAt(rax *rax, unsigned char *s, size_t len, void *data, void **old, raxNodeLink *link)
{
    ...
    if (h->iscompr && i != len) {
        ...
        raxNode *next = raxGetChild(rax, childfield);
        debugf("Next is %p\n", (void *)next);
        debugf("iskey %d\n", h->iskey);
        if (h->iskey) {
        ...
        if (j == 0) {
            ...
            raxSetChild(rax, parentlink, splitnode);
        } else {
            ...
            raxSetChild(rax, cp, splitnode);
            raxSetChild(rax, parentlink, trimmed);
        ...
        raxSetChild(rax, splitchild, postfix);
...
int raxFindLink(rax *rax, unsigned char *s, size_t len, void **value, raxNodeLink *link)
{
    debugf("### FindLink: %.*s\n", (int)len, s);
    link->splitpos = 0;
    link->consumed = raxLowWalk(rax, s, len, &link->stopnode, &link->parentlink, &link->splitpos, NULL);
     ...
     * is a key. */
    if (link->consumed != len || (link->stopnode->iscompr && link->splitpos != 0) || !link->stopnode->iskey)
        return 0;
    if (value != NULL)
        *value = raxGetData(link->stopnode);
    return 1;
}
...
int raxRemove(rax *rax, unsigned char *s, size_t len, void **old)
{
    ...
    if (h->size == 0) {
        ...
        if (child) {
            ...
            if (new != h) {
                ...
                raxSetChild(rax, parentlink, new);
            }
    ...
    if (trycompress) {
        ...
        while (1) {
            ...
            if (!parent || parent->iskey || (!parent->iscompr && parent->size != 1))
                break;
            h = parent;
...
static void raxFreeNodesWithCallback(rax *rax, raxNode *n, void (*free_callback)(void *item),
                                     void (*free_callback_withctx)(void *item, void *ctx), void *ctx)
{
    raxStack stack;
    raxStackInit(&stack);
...
/* Free a whole radix tree, calling the specified callback in order to
 * free the auxiliary data. */
void raxFreeWithCbAndContext(rax *rax, void (*free_callback)(void *item, void *ctx), void *ctx)
{
...
int raxIteratorAddChars(raxIterator *it, unsigned char *s, size_t len)
{
    ...
    if (it->key_max < it->key_len + len) {
        ...
        it->key = new_key;
        if (old == NULL)
            memcpy(it->key, it->key_static_string, it->key_len);
        it->key_max = new_max;
    }
...
int raxIteratorNextStep(raxIterator *it, int noup)
{
    ...
    while (1) {
        ...
        if (!noup && children) {
            ...
            if (!raxStackPush(&it->stack, it->node))
                return 0;
            raxNode **cp = raxNodeFirstChildPtr(it->node);
            if (!raxIteratorAddChars(it, it->node->data, it->node->iscompr ? it->node->size : 1))
                return 0;
            it->node = raxGetChild(it->rt, cp);
            /* Call the node callback if any, and replace the node pointer
             * if the callback returns true. */
            if (it->node_cb && it->node_cb(&it->node, it->privdata))
                raxSetChild(it->rt, cp, it->node);
...
int raxSeek(raxIterator *it, const char *op, unsigned char *ele, size_t len)
{
    ...
    if (eq && i == len && (!it->node->iscompr || splitpos == 0) && it->node->iskey) {
         ...
         * "equal" condition. */
        if (!raxIteratorAddChars(it, ele, len))
            return 0; /* OOM. */
        it->data = raxGetData(it->node);
    } else if (lt || gt) {
        ...
        if (i != len && !it->node->iscompr) {
            ...
            if (!raxIteratorAddChars(it, ele + i, 1))
                return 0;
...
int raxRandomWalk(raxIterator *it, size_t steps)
{
    ...
    while (steps > 0 || !n->iskey) {
        ...
        if (r == numchildren) {
            ...
            if (n->iscompr) {
                if (!raxIteratorAddChars(it, n->data, n->size))
                    return 0;
            } else {
                if (!raxIteratorAddChars(it, n->data + r, 1))
                    return 0;
            }
            raxNode **cp = raxNodeFirstChildPtr(n) + r;
            if (!raxStackPush(&it->stack, n))
                return 0;
            n = raxGetChild(it->rt, cp);
        }
        if (n->iskey)
            steps--;
    }
...
int raxCompare(raxIterator *iter, const char *op, unsigned char *key, size_t key_len)
{
    ...
    /* Handle == */
    if (lt == 0 && gt == 0)
        return cmp == 0 && key_len == iter->key_len;
