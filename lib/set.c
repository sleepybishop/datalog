...
static s_set_item *set_add_collision(s_set *set, void *data, size_t len, size_t hash, s_set_item *i)
{
...
static int set_remove_linked(s_set *set, s_set_item *item, s_set_item *i)
{
    s_set_item **j = &i->next;
    while (*j && *j != item)
        j = &(*j)->next;
...
void set_resize(s_set *set, size_t max)
{
    ...
    for (i = 0; i < set->max; i++) {
        ...
        if (!item->len)
            item = NULL;
        while (item) {
            set_add_h(&n, item->data, item->len, item->hash);
            item = item->next;
        }
    }
    set_destroy(set);
