...
// contributors: Reini Urban, Dietrich Epp, Joshua Haberman, Tommy Ettinger, Daniel Lemire, Otmar Ertl, cocowalla, leo-yuriev, Diego
// Barrios Romero, paulie-g, dumblob, Yann Collet, ivte-ms, hyb, James Z.M. Gao, easyaspi314 (Devin), TheOneric
...
static inline uint64_t w1rand(uint64_t *seed)
{
    const uint64_t c = 0xd07ebc63274654c7ull;
    *seed += c;
    return _wymix(*seed, *seed ^ c);
}
...
#if (!WYHASH_32BIT_MUM)
// fast range integer random number generation on [0,k) credit to Daniel Lemire. May not work when WYHASH_32BIT_MUM=1. It can be
// combined with wyrand, wyhash64 or wyhash.
